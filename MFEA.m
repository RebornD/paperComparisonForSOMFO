function data_MFEA = MFEA(Tasks,pop,gen,selection_process,rmp,p_il,reps,directoryname,index,mu_,sigma_)
%MFEA function: implementation of MFEA algorithm
    ...
    for rep = 1:reps
        ...
        for i = 1:no_of_tasks
            ...
            if i == 1
                EvBestFitnessone(rep,1)=bestobj(i);
            elseif i == 2
                EvBestFitnesstwo(rep,1)=bestobj(i);
            end
        ...
        mu = mu_;     % Index of Simulated Binary Crossover (tunable)
        sigma = sigma_;    % Index of polynomial mutation
        generation=1;
...
%                    child(count).rnvec(swap_indicator) = temp;
            ...
            for i = 1:no_of_tasks
                ...
                if i == 1
                    EvBestFitnessone(rep,generation)=bestobj(i);
                elseif i == 2
                    EvBestFitnesstwo(rep,generation)=bestobj(i);
                end
            end
            for i=1:2*pop
           ...
           if bestobj(1) < 1.0E-14 && bestobj(2) < 1.0E-14
              break; 
           end
       end 
    end
    ...
    dlmwrite(filename,EvBestFitnessone,'precision','%.32f');
    tasknumber = 2;
    filename = strcat(directoryname,'\','task',num2str(tasknumber),'.csv');
    dlmwrite(filename,EvBestFitnesstwo,'precision','%.32f');
